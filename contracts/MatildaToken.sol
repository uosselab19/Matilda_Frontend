...
contract MatildaToken is
    KIP17,
    KIP17Enumerable,
    KIP17MetadataMintable,
    KIP17Burnable
{
    string public _name = "Matilda Test Token";
    string public _symbol = "MTT";
    
    constructor() KIP17(_name, _symbol) {
    ...
    // 판매 등록된 토큰의 목록
    uint256[] private _onSaleTokens;

    // 판매 등록된 토큰 목록의 인덱스
    mapping(uint256 => uint256) private _onSaleTokensIndex;

    // 판매 등록된 토큰의 소유주
    mapping(uint256 => address payable) private _tokenSeller;
    ...
    function mintNFT(address _to, string memory _tokenURI) public {
        mintWithTokenURI(_to, newId, _tokenURI);
        emit tokenMinted(_to, newId, _tokenURI);
        newId += 1;
    }
    ...
    function setForSale(uint256 _tokenId, uint256 _price) public {
        ...
        require(!_isOnSale(_tokenId), "The token is already on sale");
        
        _addOnSale(_tokenId, _price);
    }

    // 판매 취소
    function unsetForSale(uint256 _tokenId) public {
        require(msg.sender == ownerOf(_tokenId), "You are not the owner");
        require(_isOnSale(_tokenId), "The token is not on sale");

        _removeOnSale(_tokenId);
    }
    
    // 판매 등록된 토큰 목록 확인
    function getOnSale() public view returns (uint256[] memory) {
        return _onSaleTokens;
    }

    // 판매 상태인 토큰의 소유자, 가격 확인
    function getOnSaleInfo(uint256 _tokenId) public view returns (address payable, uint256) {
        require(_isOnSale(_tokenId), "The token is not on sale" );
        return (_tokenSeller[_tokenId], _tokenPrice[_tokenId]);
    }
    ...
    function buyNFT(uint256 _tokenId) public payable {
        ...
        (bool sent, ) = seller.call{value: _tokenPrice[_tokenId]}("");
        require(sent, "Failed to send Klay");
        emit valueTransfered(seller, msg.sender, _tokenPrice[_tokenId]);

        _safeTransferFromPrivate(seller, msg.sender, _tokenId);

        _removeOnSale(_tokenId);
    }

    // private 함수
    function _safeTransferFromPrivate(
        address from,
        address to,
        uint256 tokenId
    ) private {
        _safeTransfer(from, to, tokenId, "");
        emit tokenTransfered(from, to, tokenId, _tokenPrice[tokenId]);
    }
    
    function _addOnSale(uint256 tokenId, uint256 price) private {
        _onSaleTokensIndex[tokenId] = _onSaleTokens.length;
        _onSaleTokens.push(tokenId);

        _tokenSeller[tokenId] = payable(msg.sender);
        _tokenPrice[tokenId] = price;

        emit saleStatusChanged(ownerOf(tokenId), tokenId, price);
    }

    function _removeOnSale(uint256 tokenId) private {
        uint256 lastTokenIndex = _onSaleTokens.length - 1;
        uint256 tokenIndex = _onSaleTokensIndex[tokenId];

        uint256 lastTokenId = _onSaleTokens[lastTokenIndex];
        _onSaleTokens[tokenIndex] = lastTokenId;
        _onSaleTokensIndex[lastTokenId] = tokenIndex;

        delete _onSaleTokensIndex[tokenId];
        _onSaleTokens.pop();

        _tokenSeller[tokenId] = payable(address(0));
        _tokenPrice[tokenId] = 0;

        emit saleStatusChanged(ownerOf(tokenId), tokenId, 0);
    }

    function _isOnSale(uint256 tokenId) private view returns (bool) {
        if(_tokenSeller[tokenId] != payable(address(0)))
            return true;
        else
            return false;
    }
    ...
    // 중복 override 처리
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override(KIP17, KIP17Enumerable) {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(KIP17, KIP17MetadataMintable, KIP17Enumerable, KIP17Burnable) returns (bool) {
        return
            KIP17.supportsInterface(interfaceId) ||
