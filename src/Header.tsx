...
export const Header = () => {
  ...
  let sign = (cookies.get("userInfo")) ? // 오른쪽 버튼 보여주기
    (<>
      <button className="btn btn-outline-light me-2" onClick={() => { fetchSignout() }}>sign-out</button>
      <button className="btn btn-secondary" onClick={() => { linkTo('/mypage') }}>My Page</button>
    </>)
    :
    (<>
     <button className="btn btn-outline-light me-2" onClick={() => { linkTo('/signin') }}>Sign-in</button>
     <button className="btn btn-secondary" onClick={() => { linkTo('/signup') }}>Sign-up</button>
    </>);
  ...
  const tapItem = (index: React.Key, title: string, url: To) => {
    return (
        <button key={index}
          className={`btn btn-outline-dark px-2 fw-bold ${pathname == url ? "text-white" : "text-secondary"} text-decoration-none`}
          onClick={() => { linkTo(url); }}>
          {title}
        </button>
    );
    ...
    <header className="p-2 bg-dark text-white sticky-top">
      <div className="container">
        <div className="d-flex flex-wrap align-items-center justify-content-between justify-content-lg-start">
          ...
          {/* lg 이상 클 때 */}
          <div className="nav col-lg-auto me-lg-auto mb-2 d-none d-lg-block">
            {tapItemList}
          </div>
