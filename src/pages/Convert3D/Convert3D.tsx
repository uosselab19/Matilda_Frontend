...
export const Convert3D = () => {
  ...
  //카테고리에 쓰일 아코디언
  const accordionItems = (index: Number, firstCategoryName: string, subCategoryNameList: Array<categoryItem>): JSX.Element => {
    //소항목에 들어갈 항목 집어넣어주는 함수
    const accordionItemLink = (subList: Array<categoryItem>): JSX.Element => {
      const linkList = [...subList].map((e, i) => {
        return (
          <li key={i} className="list-group-item">
            <button className="btn link-dark text-decoration-none" onClick={() => { handleAccordionItem(e); }}>{e.name}</button>
          </li>
        );
      });
    ...
    return (
      <div key={`key${index}`} className="accordion-item">
        ...
        {/* 소항목 분류 부분 */}
        <div id={`collapse${index}`} className="accordion-collapse collapse" aria-labelledby={`collapse${index}`} data-bs-parent="#accordionCategory">
          <div className="accordion-body">
            {accordionItemLink(subCategoryNameList)}
          </div>
        </div>
      </div>
    );
  ...
  const listAccordionItems = () => {
    ...
    result.push(accordionItems(1, "한 벌 의상(DR)", [
    ...
    result.push(accordionItems(2, "상의(TOP)", [
    ...
    result.push(accordionItems(3, "아우터(OTR)", [
    ...
    result.push(accordionItems(4, "하의(BTM)", [
    ...
    result.push(accordionItems(5, "양말(SOX)", [
    ...
    result.push(accordionItems(6, "신발류(SH)", [
    ...
    result.push(accordionItems(7, "헤어(HAIR)", [
    ...
    result.push(accordionItems(8, "헤드웨어(HEADWEAR)", [
    ...
    result.push(accordionItems(11, "액세서리", [
          ...
          <div className="image-container">
            <label htmlFor="file-input">
              ...
              <div className="card text-white border-light">
                <img id="preview-image" width="100%" height="100%" alt="" title="여기 맞아요, 사진을 넣어 주세요!"
                  src={clothes} onDrop={handleDrop} onDragOver={handleDrag} onDragLeave={handleDrag} draggable="false" />
              ...
              <input id="file-input" type="file" style={{ display: "none" }}
                accept="image/*" onChange={handleInput} />{/* gif는 들어가면 안 됨 */}
            </label>
          </div>
        </div>
