...
export const Explore = () => {
  ...
  const navItems = () => {
    const navItem = (cat_code: string, title: string) => {
      return (
        <button key={cat_code} type="button" className={`btn btn-outline-light text-dark ${(category == cat_code) ? "fw-bold" : ""}`}
          onClick={() => {
            navigate(`/explore?cat_id=${cat_code}`, { replace: false });
  ...
  return (
    <main>
      ...
      <section className="container py-5 text-center">
        <form>
          <div className="row">
            <div className="col-3 fw-bold fs-4"><img src="./matilda.png" width="64" alt=""></img> MATILDA</div>
            <input className="col-6" type="search" placeholder="Search" aria-label="Search" />
            <button className="btn btn-outline-success mx-5 col-1" type="submit">Search</button>
          </div>
        </form>
      </section>
      ...
      <section className="container py-5">
        {/* 페이지네이션 */}
        <Card
          category={category as string} page={page} setPage={setPage}
          numItems={numItems} numShowItems={numShowItems}
        />
      </section>
    </main>
  );
