...
const Pagination = (props: props) => {
	const page = props.page; const setPage = props.setPage;
	...
	const prevPage = () => {
		if (page > maxShowPage - 1) // 1 2 3 4 5 중에 하나가 되면 안 됨
			setPage(page - (page % maxShowPage + 1)); // 다음 수열 중에 가장 큰 숫자로 이동.
		// ex) 6 7 8 9 10 => 1 2 3 4 5 면 page는 4
	}
	const nextPage = () => {
		if (page + (maxShowPage - page % maxShowPage) < maxNumItems)
			setPage(page + (maxShowPage - page % maxShowPage)); // 다음 수열 중에 가장 큰 숫자로 이동.
	...
	const pagigationButton = (number: number, selected: boolean) => {
		...
		return (
			<li className="page-item" key={number}>
				<button className={`page-link link-dark ${selected ? "fw-bold" : ""}`} onClick={() => { setPage(number - 1); window.scrollTo({ top: 0 }); }}>
					{number}
				</button>
			</li>
		);
	...
	return (
		<nav aria-label="Page navigation">
			<ul className="pagination justify-content-center">
				<li className="page-item"><button className="page-link link-dark" onClick={() => { prevPage(); window.scrollTo({ top: 0 }); }}>Prev</button></li>
				{paginationManage(page)}
				<li className="page-item"><button className="page-link link-dark" onClick={() => { nextPage(); window.scrollTo({ top: 0 }); }}>Next</button></li>
			</ul>
		</nav>
	);
}
...
export const Card = (props: props) => {
	...
	const handleMouse = (e: React.MouseEvent): void => {
		...
		if (e.type === "mouseover") {
			childImg.setAttribute("style",
				"transition: width height;" +
				"transition-duration: 1.25s;" +
				"transition-timing-function: easy-in;" +
				"transform:scale(1.5,1.5);"
			);
			childEffect.setAttribute("style",
				"transition-duration: 1.25s;" +
				"transition-timing-function: easy-in;" +
			...
			childDesc.setAttribute("style",
				"transition-duration: 0.5s;" +
				"transition-timing-function: easy-in;" +
				"opacity:0;"
			);
			childTitle.setAttribute("style",
	...
	const cardItem = (info: info) => { //Main에 올라갈 card item JSX 만드는 함수
		//borderTopRightRadius: 30, borderTopLeftRadius: 30
		//borderRadius: 30
		return (
			<div key={info.id} className="col"
				onClick={() => { handleCard(info.id) }} draggable="false">
				<div className="card overflow-hidden text-white d-flex flex-column" onMouseOver={handleMouse} onMouseLeave={handleMouse}
					style={{
						borderTopRightRadius: 30, borderTopLeftRadius: 30, borderBottomRightRadius: 10, borderBottomLeftRadius: 10
					}}>
					...
					<div className="card-img-overlay d-flex flex-column">
						<div className="card-text mt-auto d-flex justify-content-between px-2 py-2">
							<div>{`${info.title}`}</div>
							<img src={`${info.maker.profile_img}`} alt="profile" width="32" height="32" className="rounded-circle me-3" />
							<div>{`${info.price} KLAY`}</div>
						</div>
					</div>
					<div className="card-img-overlay d-flex justify-content-center align-items-center mx-auto my-auto"
						style={{opacity:0}}>
						<h2>{info.title}</h2>
					</div>
				</div>
	...
	//카드 리스트
	const stateLists = new Array(numItems).fill(0).map((_, i) => {
	...
	return (
		<div>
			<div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 align-items-around g-3">
				{stateLists.slice(page * numShowItems, (page + 1) * numShowItems)}
			</div>
			<p />
			{Pagination(props)}
		</div>
